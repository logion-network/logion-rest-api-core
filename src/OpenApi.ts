...
export function setOpenApi3(spec: OpenAPIV3.Document) {
    spec.openapi = "3.0.0";
    const anySpec = spec as any; // eslint-disable-line @typescript-eslint/no-explicit-any
...
export async function loadSchemasIntoSpec(spec: OpenAPIV3.Document, path: string) {
    ...
    if(document.components !== undefined && document.components.schemas !== undefined) {
        Object.keys(document.components.schemas).forEach(schemaName =>
            addSchema(spec, schemaName, requireDefined(document.components?.schemas)[schemaName]));
    }
}
...
export function addSchema(spec: OpenAPIV3.Document, name: string, schema: OpenAPIV3.ReferenceObject | OpenAPIV3.SchemaObject) {
    ...
    requireDefined(spec.components.schemas)[name] = schema;
}
...
export function setPathParameters(operationObject: OpenAPIV3.OperationObject, params: Record<string, string>) {
    ...
    Object.keys(params).forEach(parameterName => {
        ...
        requireDefined(operationObject.parameters).push({
            name: parameterName,
            in: "path",
            description,
            required: true,
